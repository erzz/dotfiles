...
grep -q 'renderer_path="${MISE_CONFIG_ROOT:?}/bootstrap/render-private-config.sh" && mise exec fnox -- fnox exec --replace -- /bin/bash "\$renderer_path"' "$root/mise.toml"
! grep -q 'mise run render-private-config' "$root/mise.toml"
grep -q 'mise exec fnox -- fnox exec --replace -- /bin/bash "\$renderer_path" && printf.*converging apps.*&& mise exec fnox -- fnox exec --replace -- mise -E apps bootstrap' "$root/mise.toml"
! grep -q "converging apps and remaining declarations'; mise exec fnox" "$root/mise.toml"
grep -q 'mise -E apps bootstrap --force-dotfiles --yes' "$root/mise.toml"
! grep -q -- '--skip macos-defaults' "$root/mise.toml"
grep -q -- '"${MISE_CONFIG_ROOT:?}/brew/Brewfile.casks"' "$root/mise.toml"
grep -q -- '"${MISE_CONFIG_ROOT:?}/brew/Brewfile.fonts"' "$root/mise.toml"
grep -q 'mise run casks && mise run fonts' "$root/mise.toml"
